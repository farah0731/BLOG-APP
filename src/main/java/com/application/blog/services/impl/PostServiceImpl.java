...
public class PostServiceImpl implements PostService {
	...
	public PostDto createPost(PostDto postDto,Integer userId ,Integer categoryId) {
		...
		Category category = this.categoryRepo.findById(categoryId).orElseThrow(()->new ResourceNotFoundException("Categoty","category id",categoryId));
		Post post= this.modelMapper.map(postDto, Post.class);
	    post.setImageName("default.png");
		post.setAddedDate(new Date());	
	   // post.setAddDate(new java.util.Date()); // Use java.util.Date for the current date
		post.setUser(user);
		...
		if(sortDir.equalsIgnoreCase("asc"))
		  ...
		  }else
		  {
		   ...
		   Pageable p = PageRequest.of(pageNumber, pageSize, sort);
		    ...
		    // Get the content from the paginated result
		   ...
		   //  List<Post> allPosts=  this.postRepo.findAll();
		     ...
		     postResponse.setTotalElements(pagePost.getTotalElements()); // Corrected this line
	...
	public List<PostDto> searchPosts(String keyword) {
	    List<Post> posts = this.postRepo.findByTitleContaining(keyword); // Fetch the posts
	    List<PostDto> postDtos = posts.stream()
	        .map((post) -> this.modelMapper.map(post, PostDto.class))
	        .collect(Collectors.toList());
